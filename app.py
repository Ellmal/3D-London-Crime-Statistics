...
from src.transformation.aggregate_crime_grid import CRIME_COUNT_COLUMN
...
from src.visualization.hexagon_3d_map import (
    aggregate_points_to_hex,
    build_hexagon_map,
)
...
@st.cache_data
def prepare_hex_map_data(point_df: pd.DataFrame) -> pd.DataFrame:
    """Aggregate map-ready points into hex cells for the 3D layer."""
    if point_df.empty:
        return point_df
    return aggregate_points_to_hex(point_df)
...
st.caption(
    f"Exploratory crime map for **{MONTH}** — 3D hex columns or raw points, "
    "with filters that update the map and summary metrics."
)
...
with st.sidebar:
    ...
    selected_map_layer = st.radio(
        ...
        index=0,
        help=(
            "**3D hexagons** — aggregated crime intensity (Mini 10 style). "
    ...
    within_london_bbox = st.toggle(
        ...
        value=True,
        help=(
            "When on, only rows with valid coordinates inside the London bbox "
            f"are mapped. When off, any row with `{HAS_VALID_COORDINATES_COLUMN}` "
            "is included (may show points outside London)."
        ),
    )
...
hex_df = prepare_hex_map_data(map_points_df) if selected_map_layer == MAP_LAYER_HEXAGONS else pd.DataFrame()

crimes_on_map = (
    int(hex_df[CRIME_COUNT_COLUMN].sum()) if not hex_df.empty else len(map_points_df)
)
...
if filtered_df.empty:
    ...
    if map_points_df.empty:
        st.warning(
            "No map-ready points for the current filters — charts above reflect "
            "crime-type selection only."
        )
    else:
...
if filtered_df.empty:
    st.warning("No map view is available because the crime type filter returned no rows.")
elif map_points_df.empty:
    st.info(
        "No map-ready points for the current filters. "
        f"Try turning on **Limit to London bounding box** or choosing **{ALL_CRIME_TYPES_LABEL}**."
    )
else:
    if selected_map_layer == MAP_LAYER_HEXAGONS:
        st.caption(
            ...
            + (
                f" · `{HAS_VALID_COORDINATES_COLUMN}`"
                + (f" + `{IS_WITHIN_LONDON_BBOX_COLUMN}`" if within_london_bbox else "")
            )
